...
class ConfigService:
    ...
    def save_web_config(self, config_info):
        web_config = self.server.load_config_simple("config.json", DEFALUT_CONFIG, echo_in_console=False)
        action = config_info.action

        if action == "config":
            if config_info.host: web_config["host"] = config_info.host
            if config_info.port: web_config["port"] = int(config_info.port)
            if config_info.super_account: web_config["super_admin_account"] = int(config_info.super_account)
            if config_info.ai_api_key is not None: web_config["ai_api_key"] = config_info.ai_api_key
            if config_info.ai_model is not None: web_config["ai_model"] = config_info.ai_model
            if config_info.ai_api_url is not None: web_config["ai_api_url"] = config_info.ai_api_url
            if config_info.mcdr_plugins_url is not None: web_config["mcdr_plugins_url"] = config_info.mcdr_plugins_url
            if config_info.repositories is not None: web_config["repositories"] = config_info.repositories
            if config_info.ssl_enabled is not None: web_config["ssl_enabled"] = config_info.ssl_enabled
            if config_info.ssl_certfile is not None: web_config["ssl_certfile"] = config_info.ssl_certfile
            if config_info.ssl_keyfile is not None: web_config["ssl_keyfile"] = config_info.ssl_keyfile
            if config_info.ssl_keyfile_password is not None: web_config["ssl_keyfile_password"] = config_info.ssl_keyfile_password
            if config_info.public_chat_enabled is not None: web_config["public_chat_enabled"] = config_info.public_chat_enabled
            if config_info.public_chat_to_game_enabled is not None: web_config["public_chat_to_game_enabled"] = config_info.public_chat_to_game_enabled
            if config_info.chat_verification_expire_minutes is not None: web_config["chat_verification_expire_minutes"] = config_info.chat_verification_expire_minutes
            if config_info.chat_session_expire_hours is not None: web_config["chat_session_expire_hours"] = config_info.chat_session_expire_hours
            if config_info.force_standalone is not None: web_config["force_standalone"] = config_info.force_standalone
            if config_info.icp_records is not None: web_config["icp_records"] = config_info.icp_records
    ...
    def load_config(self, path: str, translation: bool = False, config_type: str = "auto"):
        try:
                ...
                if html_path.exists() and html_path.suffix == ".html":
                    try:
                        with open(html_path, "r", encoding="UTF-8") as f:
                            return {"status": "success", "type": "html", "content": f.read()}
                    except Exception:
                        return {"status": "error", "message": "Failed to read HTML file", "code": 500}
        ...
        try:
            ...
            with open(path_obj, "r", encoding="UTF-8") as f:
                raw_text = f.read()
                f.seek(0)
                if path_obj.suffix == ".json": config = json.load(f)
                    ...
                    config = {k: v if v not in ["true", "false"] else True if v == "true" else False for k, v in config.items()}
        except Exception: config = {}

        if translation:
            if path_obj.suffix == ".json":
                try: return self._maybe_nest_i18n(build_json_i18n_translations(config))
                except Exception: pass
            elif path_obj.suffix in [".yml", ".yaml"]:
                try: return self._maybe_nest_i18n(build_yaml_i18n_translations(config, raw_text or ""))
                except Exception: return get_comment(config)
    ...
    def save_config(self, file_path: str, config_data: dict):
        ...
        try:
            with open(config_path, "r", encoding="UTF-8") as f:
                if config_path.suffix == ".json": data = json.load(f)
                    ...
                    config_data = {k: v if not isinstance(v, bool) else "true" if v else "false" for k, v in config_data.items()}
            
            if config_path.suffix == ".json":
                if config_path.name == "help_msg.json" and isinstance(config_data, dict):
                    for field in ['admin_help_msg', 'group_help_msg']:
                        if field in config_data: data[field] = config_data[field]
                elif isinstance(config_data, dict) and len(config_data) == 0 and len(data) > 0:
                    data.clear()
                else:
            ...
            with open(config_path, "w", encoding="UTF-8") as f:
                if config_path.suffix == ".json": json.dump(data, f, ensure_ascii=False, indent=4)
                elif config_path.suffix in [".yml", ".yaml"]:
                    from ..utils.table import yaml
                    yaml.dump(data, f)
                elif config_path.suffix == ".properties":
    ...
    def setup_rcon(self):
        try:
            ...
            if SERVER_PROPERTIES_PATH.exists():
                ...
                mc_config.update({"enable-rcon": "true", "rcon.port": str(rcon_port), "rcon.password": rcon_password, "broadcast-rcon-to-ops": "false"})
                with open(SERVER_PROPERTIES_PATH, "w", encoding="UTF-8") as f:
                    javaproperties.dump(mc_config, f)
            else: return {"status": "error", "message": "找不到server.properties文件"}
            ...
            if config_path.exists():
                ...
                if "rcon" not in mcdr_config: mcdr_config["rcon"] = {}
                mcdr_config["rcon"].update({"enable": True, "address": rcon_host, "port": rcon_port, "password": rcon_password})
                with open(config_path, "w", encoding="UTF-8") as f:
                    yaml.dump(mcdr_config, f)
            else: return {"status": "error", "message": "找不到MCDR config.yml文件"}

            api_cache.invalidate("rcon_status")
            self.server.execute_command("!!MCDR reload config")
            return {"status": "success", "message": "RCON配置已成功启用", "config": {"rcon_host": rcon_host, "rcon_port": rcon_port, "rcon_password": rcon_password}}
        except Exception as e:
            return {"status": "error", "message": f"配置RCON时发生错误: {str(e)}"}

    def _maybe_nest_i18n(self, i18n: dict) -> dict:
        try:
            if not isinstance(i18n, dict): return i18n
        ...
        for full_key, meta in (flat_map or {}).items():
            ...
            for i, part in enumerate(parts):
                ...
                if i == len(parts) - 1 and isinstance(meta, dict):
                    if "name" in meta: cur[part]["name"] = meta.get("name")
